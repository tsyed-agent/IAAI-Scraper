...
| 0.6 | ◐ partial | **No tokens in logs.** `?api_key=` lands in uvicorn/proxy access logs; access-log redaction is in place (`logging_utils.AccessLogTokenRedactor`); replace with short-lived signed media URLs before any UI GA. | `iaai_scraper/logging_utils.py`, `cli.serve`; later `auth.py` | Grep of access logs shows no token material; signed URLs still open (Task C). |
...
**Exit:** suite green (127 tests as of 2026-07-16 after 0.6a); read path proven
write-free; redirect, eviction, negative-cache, single-flight, and access-log
redaction tests pass. Remaining: 0.6b (signed media URLs) and the 0.11 manual
container check.
