...
|---|---|---|---|
| 0.1 | **Run DDL/migrations once at startup, not per request.** `_store()` builds a new `SqliteStore` per request and `__init__` always executes DDL + `_migrate()` — including the `INSERT OR IGNORE … FROM price_history` backfill scan and an `UPDATE lots …` — i.e. write statements and a growing-table scan on every read, contending with a running crawl for the WAL write lock. | `iaai_scraper/storage.py` (`SqliteStore.__init__`, `_migrate`), `iaai_scraper/api.py` (`_store`, `_lifespan`) | Migration runs once in app lifespan (and once per CLI process); request path issues SELECTs only; test asserts no write statements on `GET /lots`. |
| 0.2 | **Thumbnail fetch must not follow redirects off the allowlist.** `urlopen` follows 3xx by default, so the host allowlist only constrains the first hop — contradicts doc 10 §6's stated SSRF mitigation. | `iaai_scraper/images.py` (`_default_fetch`) | Custom redirect handler re-validates every hop with `is_allowed_image_url` (or treat 3xx as `ImageFetchError`); test with a fake 302 to a non-allowlisted host. |
| 0.3 | **Add `source_observed_at` and `currency` columns to `price_history`** (mandated by §4), populated from `Lot.server_observed_at` / `Lot.currency`. | storage DDL, `_MIGRATIONS`, `_record_price_changes` | Migration test; new rows carry both fields. |
| 0.4 | **Stop dual-writing `status_*` rows into `price_history`.** `lot_status_history` is the only lifecycle log; filter legacy `status_*` rows out of `GET /lots/{stock}/price-history` so the price endpoint returns prices only. | `storage._record_status_change`, `api.get_price_history` | Price-history responses contain only real price types; status endpoint unchanged. |
| 0.5 | **Thumbnail cache hardening:** per-key single-flight lock (N concurrent first viewers → one upstream fetch); negative caching of upstream failures (~10 min) so a dead URL isn't re-fetched per request; stale-on-error (serve last good thumb when a refetch fails); eviction when a lot becomes `removed`/delisted plus a TTL/size sweep. | `iaai_scraper/images.py` | Meets doc 10 N5; concurrency, negative-cache, stale-serve, and eviction tests. |
| 0.6 | **No tokens in logs.** `?api_key=` lands in uvicorn/proxy access logs; add log redaction now and replace with short-lived signed media URLs before any UI GA. | logging config / gateway; later `auth.py` | Grep of access logs shows no token material. |
| 0.7 | **`/readyz` reachable by probes.** It currently requires auth; LB/k8s probes cannot send headers. Exempt it (it exposes only counts/timestamps) or make the exemption configurable. | `api.py` / `auth.py` | Unauthenticated probe works when configured. |
| 0.8 | **Fix `sold_from`/`sold_to` semantics** — they filter `status_updated_at` for *any* status. Constrain to concluded statuses or rename to `status_changed_from/to`. | `storage._build_where` | Filter matches its name; test. |
| 0.9 | **Tie `--canada-wide` page-size default to the flag** (legacy verified max is 100, not 1000). | `cli.py`, `api.CrawlCommand` | Canada-wide defaults to 100. |
| 0.10 | **Normalize `StockNum` identically** in duplicate-page detection and the parser (whitespace differences currently defeat the duplicate guard). | `crawler._fetch_page_with_recovery` | Shared normalization helper + test. |
| 0.11 | **Verify in-container crawl.** `read_only: true` with only a `/tmp` tmpfs may starve Chromium of `/dev/shm`; add a `/dev/shm` tmpfs or launch with `--disable-dev-shm-usage`. | `docker-compose.yml` / session launch args | `POST /commands/crawl` completes inside the container. |
| 0.12 | *(Deferrable)* Unchanged lots still get a full ~60-column row rewrite (including the raw blob) every crawl; update only `last_seen`/`missing_run_count` on the unchanged path. | `storage.upsert_lot` | Revisit at multi-source scale; harmless at 1.5k lots. |

**Exit:** follow-up merged; suite green; read path proven write-free; redirect,
eviction, and single-flight tests pass.
